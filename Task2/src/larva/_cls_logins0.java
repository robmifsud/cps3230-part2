...
try{
RunningClock.start();
pw = new PrintWriter("C:\\Users\\rober\\workspace\\Task2/src/output_logins.txt");
...
int _state_id_badLoginsProperty = 38;
...
if (0==1){}
		...
		if (1==0){}
		else if ((_occurredEvent(_event,68/*goodLogin*/))){
		badLogins =0 ;
_cls_logins0.pw .println ("Good login observed. Bad logins: "+badLogins );

		_state_id_badLoginsProperty = 38;//moving to state unblocked
		_goto_badLoginsProperty(_info);
		}
		else if ((_occurredEvent(_event,66/*badLogin*/)) && (badLogins <2 )){
		badLogins ++;
...
_cls_logins0.pw .println ("Bad login observed. Bad logins: "+badLogins );
		...
		_state_id_badLoginsProperty = 37;//moving to state blocked
		_goto_badLoginsProperty(_info);
		}
}
else if (_state_id_badLoginsProperty==37){
		if (1==0){}
		else if ((_occurredEvent(_event,70/*unlockAccount*/)) && (locked ==false &&lockedTime .compareTo (10 )<0 )){
		badLogins =0 ;
_cls_logins0.pw .println ("Unblocked prematurely! Entering bad state!");

		_state_id_badLoginsProperty = 36;//moving to state unlockedPrematurely
		_goto_badLoginsProperty(_info);
		}
		else if ((_occurredEvent(_event,70/*unlockAccount*/)) && (locked ==false &&lockedTime .compareTo (10 )>=0 )){
		badLogins =0 ;
...
switch(_state_id){
case 38: if (_mode == 0) return "unblocked"; else return "unblocked";
case 37: if (_mode == 0) return "blocked"; else return "blocked";
case 36: if (_mode == 0) return "unlockedPrematurely"; else return "!!!SYSTEM REACHED BAD STATE!!! unlockedPrematurely "+new _BadStateExceptionlogins().toString()+" ";
